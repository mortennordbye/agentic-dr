...
> the `/dr-build` skill (Modes 1–3 routing + gates), and the Mode-3 persona
...
- `/dr-build dry-run` → Mode 1 (generate + lint + validate; optionally plan-only)
- `/dr-build failover` → Mode 2 (generate → gated plan → gated apply → troubleshoot)
- `/dr-build fix <service>` → Mode 3 (adaptive remediation of one service)
...
(stage 5) is a future `/dr-build cleanup`.
...
`/dr-build` skill (§2). Mode 3's agent is defined when Mode 3 is built (§16.7).
...
├── skills/dr-build/        # the /dr-build entrypoint + mode routing + approval discipline
...
5. **Build order of the system itself (LOCKED).** Profile guardrails → invariant lint →
   the Orchestrator → the `/dr-build` skill, validating each on the Phase-1 roots as a known-good
   fixture.
...
5. **Clean up DR — agents, DR-scoped, explicit.** Tear down the expensive Phase-2 resources,
   ...
   `/dr-build cleanup`), never part of a build run.
